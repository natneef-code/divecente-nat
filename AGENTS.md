...
## Approved operational UX rules (2026-09-15)
- Equipment inventory starts at category, then size/model, then individual asset. Keep asset-code search, status filters, pagination, tracked history and Manager-only sequential bulk creation.
- Calendar supports Day, Week, Month and List. Month cards show activity, participants, assigned/required professionals and state; days and activities link to operational detail. Mobile must reflow without clipped controls or horizontal overflow.
- Daily staffing supports at least ten fictional professionals and Permanent, Part-time or Freelance employment type. Show every session's coverage and keep manual activity-level assignment primary; session-only overrides remain secondary.
- Boat manifests count booking participants and assigned professionals as occupants. Front Desk and Manager edit groups and seats; assigned Instructor/DM may read only their activity manifests; customers are denied. Never overwrite confirmed seats. Boat crew remains an operator decision.
- Preserve all prior workflows and never weaken role checks to make a screen easier to access.
